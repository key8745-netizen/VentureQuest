...
- `explore-g3`（定價）刻意寫得比別題長:「成本加一點」是新手預設,也是最容易把收入上限鎖死的做法。定價相關的 micro-task 分散在 explore（價值陳述、同業比價）與 operate（價格分級、誘餌方案、尾數測試）。**尾數那則寫成「記下日期,兩週後比較單量」是刻意的**——寫成實驗而不是斷言,因為參考來源宣稱的效果量（銷量增加 30-40%）沒有出處。有測試擋住 `\d+%-\d+%` 這種區間出現在文案裡。
...
- 定價內容：`explore-g3` 點名成本加成的陷阱、價值要用客人的話講、需要同業參考點、文案不得出現借來的效果量區間、尾數任務必須寫成可量測的實驗。
