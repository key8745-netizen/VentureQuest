...
- **每天都有事做**：任務分成一次性（設定類，做完就收起來）和每天／每週重複（真正推進事業的動作）。重複任務隔天或下週自動回來，所以清單不會用完。
- **實績驅動進度**：每週回顧填的真實數字會自動完成對應的過關條件（例如賣出第一個 → 「收到第 1 筆真實付款」打勾），標示「實績達成」且不能手動取消。這是唯一不能自己說了算的進度。
